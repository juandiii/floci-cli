...
public class GcpConfigProfileCommand implements Callable<Integer> {
    ...
    private int listProfiles(Printer printer, ProfileStore store) {
        try {
            ...
            if (profiles.isEmpty()) {
                printer.println(Ansi.gray("No profiles found. Create one with: floci gcp config profile create <name>"));
                return 0;
            }
    ...
    private int deleteProfile(Printer printer, ProfileStore store) {
        if (name == null) { printer.error("Profile name required."); return 1; }
        try {
            if (!store.delete(name)) { printer.error("Profile '" + name + "' not found."); return 1; }
