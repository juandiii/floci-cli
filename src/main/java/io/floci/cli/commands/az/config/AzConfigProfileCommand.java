...
public class AzConfigProfileCommand implements Callable<Integer> {
    ...
    private int listProfiles(Printer printer, ProfileStore store) {
        try {
            ...
            if (profiles.isEmpty()) {
                printer.println(Ansi.gray("No profiles found. Create one with: floci az config profile create <name>"));
                return 0;
            }
