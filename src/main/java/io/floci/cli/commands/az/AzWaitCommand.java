...
public class AzWaitCommand implements Callable<Integer> {
    ...
    public Integer call() {
        ...
        while (Instant.now().isBefore(deadline)) {
            if (isReady(client, service)) {
                if (printer.format() != OutputFormat.text) {
                    ...
                    printer.println(Ansi.green("Floci Azure is ready") + " (" + effectiveEndpoint + ")");
                }
                return 0;
            }
        ...
        printer.error("Timed out waiting for Floci Azure after " + timeout + ".\nIs the container running? Try 'floci az status'.");
        return 1;
    }
