...
public class AzGlobalOptions {

    @Option(names = {"--endpoint"},
            description = "Floci Azure server endpoint URL",
            defaultValue = "${FLOCI_AZ_ENDPOINT:-http://localhost:4577}",
            paramLabel = "<url>")
    public String endpoint;

    @Option(names = {"--container"},
            description = "Floci Azure container name",
            defaultValue = "${FLOCI_AZ_CONTAINER:-floci-az}",
            paramLabel = "<name>")
    ...
    public String endpointFromPorts(String ports, String fallback) {
        ...
        try {
            ...
            if (containerPort == -1) containerPort = 4577;
